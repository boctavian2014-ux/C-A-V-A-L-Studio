/**
 * Lot C3 — MCP child env must NOT inherit full process.env (bypasses Lot B).
 * Base: Lot B sanitizeEnvForTerminal, then only system allowlist + server-declared keys.
 */
import { sanitizeEnvForTerminal, isSecretEnvKey } from "../../src/main/subprocess-env";

/** System keys MCP servers need to resolve binaries (PATH etc.). Never includes secrets. */
export const MCP_BASE_ENV_KEYS = new Set([
  "PATH",
  "Path",
  "PATHEXT",
  "HOME",
...
export type McpSecretEnvKey = (typeof MCP_SECRET_ENV_KEYS)[number];

const DEFAULT_SECRET_KEYS_BY_SERVER: Record<string, readonly string[]> = {
...
/**
 * Build env for an MCP stdio child.
 * - Does NOT spread process.env
 * - Starts from Lot B sanitize, then keeps only MCP_BASE_ENV_KEYS
 * - Adds non-empty config.env values
 * - Injects secrets from provider ONLY for keys declared on that server
 *   (config.env keys and/or known per-server secret placeholders)
 */
export function mergeMcpServerEnv(
  ...
  const sanitized = sanitizeEnvForTerminal(sourceEnv);
  ...
  const declaredKeys = new Set<string>([
    ...
    ...(serverId ? DEFAULT_SECRET_KEYS_BY_SERVER[serverId] ?? [] : []),
  ]);
  ...
  // Final safety: never leave secret keys that weren't explicitly declared for this server.
  for (const key of Object.keys(env)) {
    if (isSecretEnvKey(key) && !declaredKeys.has(key)) {
      delete env[key];
    }
  }
