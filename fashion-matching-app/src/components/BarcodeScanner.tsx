...
export const BarcodeScanner: React.FC<Props> = ({ onResult, onBack }) => {
  const camera = useRef<Camera>(null);
  const device = useCameraDevice('back');
  const { hasPermission, requestPermission } = useCameraPermission();
  const { scanned, barcode, onBarcodeDetected, resetScan } = useBarcodeScanner();
      ...
      >
        <View style={styles.overlay}>
          <View style={styles.topBar}>
            ...
            <Text style={styles.title}>Scanare Barcode</Text>
            <View style={{ width: 60 }} />
          </View>
          ...
          <View style={styles.scanFrame}>
            ...
            <Text style={styles.scanHint}>Poziționează codul de bare în interiorul dreptunghiului</Text>
          </View>
            ...
            {(processing || scanned) && (
              <View style={styles.processingContainer}>
                ...
                <Text style={styles.processingText}>
                  {scanned ? 'Barcode detectat!' : 'Procesez...'}
                </Text>
              </View>
            )}
          </View>
